...
const gameResultMessage = (ourTeam, winningTeam) => {
  ...
  if(ourTeam === 'bos'){
    return ourTeam === winningTeam ? `Damn, the universe hath no justice.` :
      `Huzzah, the good guys ${winningTeam} won!`;
  } else {
    return ourTeam === winningTeam ? `Huzzah, the good guys ${ourTeam} won!` :
      `Damn, the universe hath no justice.`;
  }
...
mlbgames.get((err, games) => {
  ...
  let myTeam = 'sf';
  let yesterdaysGame = findGamesForTeam(myTeam, games);
  if(yesterdaysGame){
